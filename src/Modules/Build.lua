...
function buildMode:InitOrImport(dbFileName, buildName, buildXML, overrideWithNew, convertBuild)
	...
	--Trying to avoid recreating controls if loading build
    if overrideWithNew == True
	...
	--else--Importing additional build into build file (this section for stuff that needs to be overrided or added when adding extra build)
	
	end

	--Not sure if need to defer the PassiveTrees if TreeData already initialized
	...
	else-- Code Section here for loading data from file (Need to attempt not to override current data with new data--Add suffix if conflicting build names) 
		for _, node in ipairs(deferredPassiveTrees) do
			-- Check if there is a saver that can load this section
			--ToDo:Adjust the load code here
			if self.treeTab:Load(node, self.dbFileName) then
				self:CloseBuild()
				return
			end
